...
const SocialIcon = ({ type, url }: { type: SocialType; url: string }) => (
  <Link
    ...
    className="flex w-[35px]  hover:bg-dark text-dark-grey hover:text-white  h-[35px] p-2 border-1 border-dark-grey/25 items-center justify-center rounded-full transition-all duration-300"
  >
    {SOCIAL_ICONS[type]}
  </Link>
);
...
export function SocialIcons({ socials = {}, contacts = {} }: SocialIconsProps) {
  return (
    <div className="flex  justify-center sm:gap-4 gap-2 max-[350px]:flex-wrap">
      {/* Social Media Icons */}
      {Object.entries(socials)
        ...
        .map(([key, url]) => (
          <SocialIcon
            key={`social-${key}`}
            type={key.toLowerCase() as SocialType}
            url={url}
          />
        ))}
