...
const FilterIcons: FilterIcons = {
  location: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="h-5 w-5 opacity-70">
      ...
      />
      <path
        d="M10 18.3333C13.3333 15 16.6667 12.0152 16.6667 8.33333C16.6667 4.65143 13.6813 1.66667 10 1.66667C6.3181 1.66667 3.33333 4.65143 3.33333 8.33333C3.33333 12.0152 6.66667 15 10 18.3333Z"
        stroke="currentColor"
        strokeWidth="1.5"
      />
    </svg>
  ...
  category: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="h-5 w-5 opacity-70">
      <path
        d="M7.5 3.33333H4.16667C3.24619 3.33333 2.5 4.07952 2.5 5V7.5C2.5 8.42047 3.24619 9.16667 4.16667 9.16667H7.5C8.42047 9.16667 9.16667 8.42047 9.16667 7.5V5C9.16667 4.07952 8.42047 3.33333 7.5 3.33333Z"
        stroke="currentColor"
        strokeWidth="1.5"
      />
      <path
        d="M15.8333 3.33333H12.5C11.5795 3.33333 10.8333 4.07952 10.8333 5V7.5C10.8333 8.42047 11.5795 9.16667 12.5 9.16667H15.8333C16.7538 9.16667 17.5 8.42047 17.5 7.5V5C17.5 4.07952 16.7538 3.33333 15.8333 3.33333Z"
        stroke="currentColor"
        strokeWidth="1.5"
      />
      <path
        d="M7.5 10.8333H4.16667C3.24619 10.8333 2.5 11.5795 2.5 12.5V15C2.5 15.9205 3.24619 16.6667 4.16667 16.6667H7.5C8.42047 16.6667 9.16667 15.9205 9.16667 15V12.5C9.16667 11.5795 8.42047 10.8333 7.5 10.8333Z"
        stroke="currentColor"
        strokeWidth="1.5"
      />
      <path
        d="M15.8333 10.8333H12.5C11.5795 10.8333 10.8333 11.5795 10.8333 12.5V15C10.8333 15.9205 11.5795 16.6667 12.5 16.6667H15.8333C16.7538 16.6667 17.5 15.9205 17.5 15V12.5C17.5 11.5795 16.7538 10.8333 15.8333 10.8333Z"
        stroke="currentColor"
        strokeWidth="1.5"
      />
    </svg>
...
const DynamicDropdown = React.lazy(() =>
  import("../sharedUi/Filters/dynamic-dropdown")
    ...
    .catch(() => ({
      default: () => <div className="p-2 text-sm text-red-500">Error loading filter</div>,
    })),
)
...
const PriceRangeDropdown = React.lazy(() =>
  import("../sharedUi/Filters/price-range-dropdown")
    .then((module) => ({ default: module.default }))
    .catch(() => ({
      default: () => <div className="p-2 text-sm text-red-500">Error loading price</div>,
    })),
)
...
const DropdownWrapper: React.FC<DropdownWrapperProps> = ({ children }) => (
  <React.Suspense fallback={<div className="h-12 animate-pulse rounded bg-gray-200" />}>
    {children}
  </React.Suspense>
)
...
const ResetButton: React.FC<ResetButtonProps> = React.memo(
  ({ hasActiveFilters, onReset, title }) => (
    <button
      onClick={onReset}
      className={`flex h-12 w-12 cursor-pointer items-center justify-center border border-gray-300 bg-white px-3 text-gray-700 transition-all hover:border-gray-400 hover:bg-gray-50 hover:text-gray-900 focus:ring-2 focus:ring-gray-300 focus:outline-none lg:w-fit ${
        hasActiveFilters ? "ring-dark/90 ring-opacity-50 animate-pulse ring-2" : ""
      } `}
      title={title}
      aria-label={title}
    >
...
const SearchButton: React.FC<SearchButtonProps> = React.memo(({ disabled, onClick, label }) => (
  <Button
    onClick={onClick}
    disabled={disabled}
    className="h-12 w-full cursor-pointer items-center justify-center gap-2 rounded-none bg-black text-sm text-white transition-colors hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50 lg:flex-1 lg:text-lg"
    aria-label={label}
  >
    {FilterIcons.search}
...
const ActionButtons: React.FC<ActionButtonsProps> = React.memo(
  ({ hasCountry, hasActiveFilters, onSearch, onReset, t }) => (
    <div className="flex items-center gap-2 ">
      <div className="flex-1 ">
        <SearchButton disabled={!hasCountry} onClick={onSearch} label={t("search")} />
      </div>
      <ResetButton
...
const HomeFilter: React.FC = () => {
  ...
  return (
    <div
      className="relative z-20 mx-auto w-full bg-white p-4 shadow-lg lg:py-6"
      role="search"
